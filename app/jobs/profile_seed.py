...
STRONG_TITLE_PATTERNS: PatternSet = (
    ...
    ("konstruktionsingenieur", re.compile(r"\bkonstruktions?(?:ingenieur|techniker)\w*")),
    ...
    ("mechanical_design_engineer", re.compile(r"\bmechanical\s+design\s+engineer\w*")),
    ("cad_konstrukteur", re.compile(r"\bcad[-\s]*(?:konstrukteur|designer)\w*")),
    ("vehicle_development_engineer", re.compile(r"\b(?:vehicle|automotive|fahrzeug)\w*\s+(?:development\s+)?engineer\w*")),
    ("rail_vehicle_engineer", re.compile(r"\b(?:rail|rolling\s+stock|schienenfahrzeug)\w*\s+(?:engineer|ingenieur)\w*")),
...
ADJACENT_ROLE_PATTERNS: PatternSet = (
    ("technical_project_lead", re.compile(r"\b(?:technical|technisch\w*)\s+(?:project|projekt)\s*(?:lead|leiter|manager)\w*")),
    ("project_engineer", re.compile(r"\b(?:project\s+engineer|projektingenieur)\w*")),
    ("project_manager", re.compile(r"\b(?:project\s+manager|projektleiter|projektmanager)\w*")),
    ...
    ("technischer_spezialist", re.compile(r"\btechnisch\w*\s+(?:spezialist|experte)\w*")),
...
DOMAIN_PATTERNS: PatternSet = (
    ("maschinenbau", re.compile(r"\bmaschinenbau\w*|\bmachine\s+(?:building|engineering)\b")),
    ...
    ("manufacturing", re.compile(r"\b(?:fertigung|manufactur|production\s+engineering)\w*")),
    ("product_development", re.compile(r"\b(?:produktentwick|product\s+development)\w*")),
    ("component_development", re.compile(r"\b(?:baugruppe|bauteil|component\s+development|mechanical\s+component)\w*")),
    ("chassis_structure", re.compile(r"\b(?:chassis|fahrgestell|wagenkasten|carbody|vehicle\s+structure)\w*")),
    ("wheel_development", re.compile(r"\b(?:radentwicklung|wheel\s+development|wheel\s+design)\w*")),
    ("stage_systems", re.compile(r"\b(?:bühnentechnik|buehnentechnik|stage\s+(?:systems?|engineering))\w*")),
)
...
METHOD_TOOL_PATTERNS: PatternSet = (
    ...
    ("strength_analysis", re.compile(r"\b(?:festigkeitsberechnung|festigkeitsbewertung|strength\s+(?:analysis|calculation|assessment))\w*")),
    ...
    ("requirements_specification", re.compile(r"\b(?:lastenheft|pflichtenheft|requirements?\s+(?:specification|engineering|management))\w*")),
    ("technical_drawing", re.compile(r"\b(?:technisch\w*\s+zeichnung|technical\s+drawing)\w*")),
    ("concept_development", re.compile(r"\b(?:konzeptentwick|concept\s+development|conceptual\s+design)\w*")),
    ("validation", re.compile(r"\b(?:validier|validation|verification)\w*")),
    ("testing", re.compile(r"\b(?:versuch|testing|test\s+engineering|prototype\s+test)\w*")),
    ("commissioning", re.compile(r"\b(?:inbetriebnahme|commissioning)\w*")),
    ("supplier_coordination", re.compile(r"\b(?:lieferanten(?:steuerung|koordination|betreuung)|supplier\s+(?:management|coordination|development))\w*")),
    ("interface_management", re.compile(r"\b(?:schnittstellenmanagement|interface\s+management)\w*")),
    ("milestone_planning", re.compile(r"\b(?:meilenstein|milestone|terminsteuerung|schedule\s+management)\w*")),
    ("series_readiness", re.compile(r"\b(?:serienreife|serienproduktion|series\s+(?:readiness|production))\w*")),
)
...
NEGATIVE_CONTEXT_PATTERNS: PatternSet = (
    ("software", re.compile(r"\b(?:software|backend|frontend|full[-\s]*stack|devops|cloud|kubernetes)\w*")),
    ("data_ai", re.compile(r"\b(?:data\s+(?:scientist|engineer)|machine\s+learning|artificial\s+intelligence|\bai\b)\w*")),
    ("enterprise_it", re.compile(r"\b(?:crm|salesforce|java|javascript|typescript|python\s+developer|sap\s+(?:developer|consultant))\w*")),
    ("sales", re.compile(r"\b(?:sales|vertrieb|business\s+development|account\s+manager)\w*")),
    ("hr", re.compile(r"\b(?:human\s+resources|recruit|talent\s+acquisition|hr\s+)\w*")),
    ("finance", re.compile(r"\b(?:accountant|accounting|finance|financial|controlling)\w*")),
)
