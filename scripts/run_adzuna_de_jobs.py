...
from app.sources.job.adzuna import BASE_URL
from app.sources.job.adzuna_de import AdzunaGermanyJobSource

SOURCE_NAME = "adzuna-api-de"
ADAPTER_PATH = "app.sources.job.adzuna_de.AdzunaGermanyJobSource"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Germany engineering frontier through Adzuna's official API."
    )
...
def get_or_create_source() -> int:
    with SessionLocal() as session:
        ...
        config = {
            "country_code": "DE",
            "scope": "Germany engineering jobs from the documented Adzuna API country=de endpoint",
            "acquisition": "official Adzuna API; targeted title queries; no advertiser scraping",
...
async def async_main() -> int:
    ...
    adapter = AdzunaGermanyJobSource(
        app_id=app_id,
        app_key=app_key,
    ...
    with SessionLocal() as session:
        ...
        if source is None:
            raise RuntimeError("Adzuna DE source disappeared before the run started")
